...
from model.gcnn_model import GCNNModel


class GCNNSupervisor(object):
    """
    ...
    def __init__(self, **kwargs):
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('GCNN', reuse=False):
                self._train_model = GCNNModel(is_training=True, scaler=scaler,
                                               batch_size=self._data_kwargs['batch_size'],
                                                **self._model_kwargs)

        with tf.name_scope('Test'):
            with tf.variable_scope('GCNN', reuse=True):
                self._test_model = GCNNModel(is_training=False, scaler=scaler,
    ...
    def run_epoch_generator(self, sess, model, data_generator, return_output=False, training=False, writer=None):
        ...
        for _, (x, y) in enumerate(data_generator):
            ...
            losses.append(vals['loss']) # include the whole training dataset
            maes.append(vals['mae'])
            regs.append(vals['reg'])
    ...
    def _train(self, sess, base_lr, epoch, steps, patience=50, epochs=100,
        ...
        while self._epoch <= epochs:
            ...
            test_gdt = test_gdt.reshape(test_gdt.shape[0], -1, order = 'F') # record the corresponding steps for each node first, then node by node
    ...
    def evaluate(self, sess, **kwargs):
        ...
        outputs = {
            ...
            'observed': self._data['y_test'],
            'groundtruth': self._data['y_test_groundtruth']#y_truths
        }
        return outputs
